...
export const Generator: React.FC<GeneratorProps> = ({ initialUrl, onUrlConfirm, onReset }) => {
  ...
  // Sync internal state if props change (e.g. from localStorage load)
  useEffect(() => {
    if (initialUrl) {
      ...
      <div className="max-w-md w-full space-y-8 animate-fade-in">
        ...
        {!isConfirmed ? (
          ...
          <div className="bg-slate-800 border border-slate-700 p-8 rounded-3xl shadow-2xl text-center">
             ...
             {/* Logo Section - Optimized for Quality */}
             <div className="flex flex-col items-center mb-8">
                <div className="relative w-28 h-28 bg-white rounded-[22px] mb-4 flex items-center justify-center shadow-2xl ring-1 ring-white/10 p-0 overflow-hidden">
                   {/* Background layer to prevent transparency issues */}
                ...
                <h2 className="text-2xl font-bold text-white tracking-tight">
                  {activeUrl ? new URL(activeUrl).hostname.replace('www.', '') : 'App Pronto'}
                </h2>
             ...
             <div className="space-y-4 text-left">
                <div className="bg-slate-900/50 border border-slate-700 p-5 rounded-2xl">
                    <h3 className="font-bold text-blue-400 flex items-center gap-2 mb-3 text-sm uppercase tracking-wider">
                    ...
                    <ol className="list-none space-y-3 text-sm text-slate-300">
                        ...
                        <li className="flex items-start gap-3">
                           <span className="flex-shrink-0 flex items-center justify-center w-6 h-6 rounded-full bg-slate-700 text-xs font-bold text-white">3</span>
                           <span>Confirme o nome e abra o novo ícone na sua tela!</span>
                        </li>
                    </ol>
                </div>
             </div>
